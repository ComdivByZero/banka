...
func help() {
	fmt.Println(
		...
		"	Пример работы:\n" +
		"1)	banka -port=4000 -name=world 127.0.0.1 &\n" +
		"2)	banka -name=man 127.0.0.1:4000 &\n" + 
		"	alias w='banka world'\n" + 
		"	w 'Hello world!'\n" +
		"1)	banka -port=4000 -exit\n" +
		"2)	banka -exit")
	flag.PrintDefaults()
}
...
func parse(f *flags) {
	const (
		...
		mh = "команда, выполняемая при получении сообщения, например,\n" + 
			 "	notify-send; параметры: 1 - автор, 2 - сообщение"
	)
	flag.Usage = help
